param(
    [string]$artifact_server = $env:ARTIFACT_SERVER,
...
#build a template
$content = "[default]`n[[IPADDRESS]]`n`n[default:vars]`nansible_connection=winrm`nansible_port=[[PORT]]`nansible_winrm_transport=basic`nansible_user=[[USER]]`nansible_password=[[PASS]]`nansible_winrm_message_encryption=auto`nansible_winrm_server_cert_validation=[[INSEC]]`nartifact_server=[[ARTSERVER]]`n"
...
$content = $content.replace('[[ARTSERVER]]',$artifact_server)
